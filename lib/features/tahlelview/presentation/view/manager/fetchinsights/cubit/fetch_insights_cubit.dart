  ...
  try {
    ...
    if (user != null) {
      ...
      if (docSnapshot.exists) {
        ...
        final purchaseSummary = data['purchase_summary'] ?? {};  // Fetch purchase summary
        ...
        List<double> purchaseData =
            _getPurchaseDataForPeriod(purchaseSummary, firestorePeriod);  // Fetch purchase data

       
        emit(FetchInsightsSuccess(salesData, purchaseData));  // Emit both data
      } else {
...
List<double> _getPurchaseDataForPeriod(Map<String, dynamic> purchaseSummary, String period) {
  // Check if the period exists in the purchase summary
  if (!purchaseSummary.containsKey(period) || purchaseSummary[period] == null) {
  ...
  List<double> completePurchaseData = List.filled(period == 'weekly' ? 7 : 12, 0.0);
  ...
  if (period == 'weekly') {
      ...
      if (entry != null && entry is Map<String, dynamic>) {
        ...
        if (dayString != null) {
          ...
          if (dayIndex >= 0 && dayIndex < 7) {
            completePurchaseData[dayIndex] = (entry['purchase'] as num?)?.toDouble() ?? 0.0;
          }
        }
      }
    }
    ...
    for (var entry in periodData) {
      if (entry != null && entry is Map<String, dynamic>) {
        String? weekString = entry['period'];  // Assuming 'week' is used for weekly data
        if (weekString != null) {
          int weekIndex = _getWeekIndexFromString(weekString);
          if (weekIndex >= 0 && weekIndex < 4 && weekIndex < currentWeek) {
            completePurchaseData[weekIndex] = (entry['purchase'] as num?)?.toDouble() ?? 0.0;
          }
        }
      }
    }
      ...
      if (entry != null && entry is Map<String, dynamic>) {
        ...
        if (monthString != null) {
          ...
          if (monthIndex >= 0 && monthIndex < 12) {
            completePurchaseData[monthIndex] = (entry['purchase'] as num?)?.toDouble() ?? 0.0;
          }
        }
      }
    }
  ...
  List<double> _getSalesDataForPeriod(Map<String, dynamic> salesSummary, String period) {
  ...
  List<double> completeSalesData = List.filled(period == 'weekly' ? 7 : 12, 0.0);
    ...
    for (var entry in periodData) {
      if (entry != null && entry is Map<String, dynamic>) {
        ...
        if (dayString != null) {
          int dayIndex = _getDayIndexFromString(dayString);
          if (dayIndex >= 0 && dayIndex < 7) {
            completeSalesData[dayIndex] = (entry['sales'] as num?)?.toDouble() ?? 0.0;
          }
        }
      }
    }
    ...
    for (var entry in periodData) {
      if (entry != null && entry is Map<String, dynamic>) {
        ...
        if (weekString != null) {
          int weekIndex = _getWeekIndexFromString(weekString);
          if (weekIndex >= 0 && weekIndex < 4 && weekIndex < currentWeek) {
            completeSalesData[weekIndex] = (entry['sales'] as num?)?.toDouble() ?? 0.0;
          }
        }
      }
    }
    ...
    for (var entry in periodData) {
      if (entry != null && entry is Map<String, dynamic>) {
        ...
        if (monthString != null) {
          int monthIndex = _getMonthFromString(monthString);
          if (monthIndex >= 0 && monthIndex < 12) {
            completeSalesData[monthIndex] = (entry['sales'] as num?)?.toDouble() ?? 0.0;
          }
        }
      }
    }
 ...
 int _getCurrentWeekOfMonth() {
  ...
  return ((dayOfMonth - 1) ~/ 7) + 1; // Divide by 7 and add 1 to get week number
}
